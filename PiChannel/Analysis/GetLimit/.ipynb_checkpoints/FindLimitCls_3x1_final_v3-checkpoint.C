...
Double_t getLimit(Double_t lum, Int_t type)
{
 ...
 /*if(type==3)
  ...
  //RooRealVar NsmR("NsmR", "fraction sm", Nsm, 0, 3*NevtsGen);
  //RooRealVar NsmbR("NbsmR", "fraction", NevtsGen , 0 , 3*NevtsGen);
  ...
  //RooAddPdf smb_pdf("smb_pdf", "tau+Bkg", RooArgList(*sm_pdf,  *b_pdf), RooArgList(NsmR, NbR));
  //RooAddPdf smb_pdf("ssmb_pdf", "Signal+tau+Bkg", RooArgList(*sm_pdf,  *bsm_pdf), RooArgList(NenunuR, NbsmR));
  ...
  if (type == 1){
   ...
   /*   
    if (lum == 100){
        ...
        //m_alpha = 0.7, 1.0
        //w.var("mu")->setMax(5.5e-2);
        //m_alpha = 1.2
        //w.var("mu")->setMax(2.5e-2);
        //m_alpha = 1.4
        //w.var("mu")->setMax(2.0e-2);
        //m_alpha = 1.6
        w.var("mu")->setMax(1.0e-2);
    }
  ...
  if (type == 2){
    ...
    if (lum == 50000){
       ...
       //w.factory("mu[0,1]");
        ...
        //w.var("mu")->setVal(1.0e-5);
        w.var("mu")->setVal(0);
        //w.var("mu")->setMin(1.0e-5);
 
        
        // to m_alph = 0.0, 0.5, 0.7, 1.0
        //w.var("mu")->setMax(5.0e-2);
        // to m_alpha = 1.2, 1.4
        //w.var("mu")->setMax(3.0e-2);
        // to m_alpha = 1.6
        //w.var("mu")->setMax(1.0e-2);
...
Double_t getLimit2D(Double_t lum)
{
 ...
 //RooAddPdf smb_pdf("smb_pdf", "Signal+tau+Bkg", RooArgList(*sm_pdf,  *b_pdf), RooArgList(NenunuR, NsmR));
  ...
  //w.factory("SUM::model(S*s_pdf,Snu*sm_ pinu_pdf,Stau*sm_tau_pdf,B*b_pdf)") ;
  //w.factory("SUM::model(S*s_pdf,Snu*pi_taunu_pdf)") ;
  //w.factory("SUM::model(S*s_pdf,Snu*pi_taunu_pdf,B*b_pdf)") ;
  w.factory("SUM::model(mu*s_pdf,Snu*pi_taunu_pdf,B*b_pdf)") ;

...
void FindLimitCls_BToInvCombinedFC()
{
  SetData();
  Double_t limit[4];
  //Double_t lum = 361.6; 
 ...
 if(type==3)
  ...
  if (type == 1){
    ...
    if (lum == 50000){
      ...
      //w.var("mu")->setMin(-4e-6);
      // to m_X = 0.0
      //w.var("mu")->setMin(-8.4e-7);
      // to m_X = 2.0
  ...
  if (type == 2){
    ...
    if (lum == 50000){
     ...
     // to m_X = 0.0
      //w.var("mu")->setMin(-12.0e-7);
      // to m_X = 2.0
      w.var("mu")->setMin(-0.6e-6);
      // to m_x = 5.0
      //w.var("mu")->setMin(-2.0e-6);
  ...
  if (type == 3){
    ...
    if (lum == 50000){
      ...
      w.factory("mu[4e-8, -1.0,1.0]");
      //w.factory("S[0.0,0.0,10.0]");
      w.factory("B[8284.0,7500.0,8700.0]");
  ...
  // Create a Feldman-Cousins calculator for the signal+background hypothesis
  double confidenceLevel = 0.95;
  int nPointsToScan = 12;
  int nToyMC = 150;

  RooStats::FeldmanCousins fc(*hist_data, b_modelNM, sb_modelNM);
  ...
  //RooStats::LikelihoodInterval *interval = fc.GetInterval();;
 
  // show use of ProfileLikeihoodCalculator utility in RooStats

  RooStats::ProfileLikelihoodCalculator plc(*hist_data, sb_modelNM);
  plc.SetTestSize(.1);
  double CL = 0.683;
  plc.SetConfidenceLevel(CL);
  RooStats::LikelihoodInterval *plcInterval = plc.GetInterval();

  RooStats::ProfileLikelihoodCalculator plc1(*hist_data, sb_modelNM);
  plc1.SetTestSize(.1);
  double CL1 = 0.90;
  plc1.SetConfidenceLevel(CL1);
  RooStats::LikelihoodInterval *plcInterval1 = plc1.GetInterval();

  RooStats::ProfileLikelihoodCalculator plc2(*hist_data, sb_modelNM);
  plc2.SetTestSize(.1);
  double CL2 = 0.95;
  plc2.SetConfidenceLevel(CL2);
  RooStats::LikelihoodInterval *plcInterval2 = plc2.GetInterval();
  ...
  // Perform the profile likelihood calculation
  RooStats::LikelihoodInterval* LInterval = plCalc.GetInterval();

  if (LInterval) {
      double lowerLimitL = LInterval->LowerLimit(*poi);
      double upperLimitL = LInterval->UpperLimit(*poi);
  ...
  TH2F *hist = (TH2F *)parameterScan->createHistogram("mu:B", npoints, npoints);
      //hist->Draw();
  ...
  for (Int_t i = 0; i < parameterScan->numEntries(); ++i) {
    ...
    if (interval) {
      if (interval->IsInInterval(*tmpPoint)) {
        forContour->SetBinContent(hist->FindBin(tmpPoint->getRealValue("mu"), tmpPoint->getRealValue("B")), 1);
      } else {
          forContour->SetBinContent(hist->FindBin(tmpPoint->getRealValue("mu"), tmpPoint->getRealValue("B")), 0);
        }
    }
  ...
  RooStats::LikelihoodIntervalPlot *plotInt = new RooStats::LikelihoodIntervalPlot(plcInterval);
  plotInt->SetTitle("Confidence Intervals");
  //plotInt->SetLineWidth(2);
  plotInt->SetLineColor(kGreen);

  //LikelihoodIntervalPlot plotInt1((LikelihoodInterval *)plcInterval1);
  RooStats::LikelihoodIntervalPlot *plotInt1 = new RooStats::LikelihoodIntervalPlot(plcInterval1);
  plotInt1->SetTitle("90% Confidence Intervals");
  //plotInt1->SetLineStyle(10);
  plotInt1->SetLineColor(kOrange);

  //LikelihoodIntervalPlot plotInt2((LikelihoodInterval *)plcInterval2);
  RooStats::LikelihoodIntervalPlot *plotInt2 = new RooStats::LikelihoodIntervalPlot(plcInterval2);
  plotInt2->SetTitle("95% Confidence Intervals");
  //plotInt2->SetLineStyle(5);
  plotInt2->SetLineColor(kCyan);
  ...
  //RooStats::LikelihoodIntervalPlot *plot = new RooStats::LikelihoodIntervalPlot(plcInterval2);
  //plot->SetNPoints(50); 
  //plot->Draw("TF1"); gPad->Draw();


  // Estimation of the UL using Feldman-Cousins
  // -------------------------------------------------------

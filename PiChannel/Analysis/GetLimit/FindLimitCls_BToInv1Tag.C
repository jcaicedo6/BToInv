...
Double_t getLimit(Double_t lum, Int_t type, Int_t type_tag);
...
// For pdf   ------
TH1D *hS_Mmin2pdf = new TH1D("hS_Mmin2pdf","hS_Mmin2pdf",nbins,M_min2Low,M_min2Up);
...
TH1D *hBkgMmin2pdf = new TH1D("hBkgMmin2pdf","hBkgMmin2pdf",nbins,M_min2Low,M_min2Up);
...
TH1D *hS_Mmax2pdf = new TH1D("hS_Mmax2pdf","hS_Mmax2pdf",nbins,M_max2Low,M_max2Up);
...
TH1D *hBkgMmax2pdf = new TH1D("hBkgMmax2pdf","hBkgMmax2pdf",nbins,M_max2Low,M_max2Up);
...
TH1D *hS_Xpspdf = new TH1D("hS_Xpspdf","hS_Xpspdf",nbins,Xps_Low,Xps_Up);
...
TH1D *hBkgXpspdf = new TH1D("hBkgXpspdf","hBkgXpspdf",nbins,Xps_Low,Xps_Up);
...
void FindLimitCls_BToInv1Tag()
{
  ...
  //limit[0] = getLimit(lum,1,type_tag);
  //limit[1] = getLimit(lum,2,type_tag);
  limit[2] = getLimit(lum,3,type_tag);
...
Double_t getLimit(Double_t lum, Int_t type, Int_t type_tag)
{
  ...
  if(type==1)
    {
      if (type_tag==1)
      {
        hS_Mmin2pdf = (TH1D*)fSDMmin2pdf->Get("Mmin2");
        hBkgMmin2pdf = (TH1D*)fBkgStagDMmin2pdf->Get("Mmin2");
      }

      if (type_tag==2)
      {
        hS_Mmin2pdf = (TH1D*)fSDstarGammaMmin2pdf->Get("Mmin2");
        hBkgMmin2pdf = (TH1D*)fSDstarGammaMmin2pdf->Get("Mmin2");
      }
      
      if (type_tag==3)
      {
        hS_Mmin2pdf = (TH1D*)fSDstarPi0Mmin2pdf->Get("Mmin2");
        hBkgMmin2pdf = (TH1D*)fBkgStagDstarPi0Mmin2pdf->Get("Mmin2");
      }


      Ns = hS_Mmin2pdf->GetEntries();
      Nb = hBkgMmin2pdf->GetEntries();
      ...
      dS = new RooDataHist("dS", "dS", *x, Import(*hS_Mmin2pdf));  // B->Pi+alpha
      dB = new RooDataHist("dB", "dB", *x, Import(*hBkgMmin2pdf));  // BKG
    }
  ...
  if(type==2)
    { 
      if (type_tag==1)
      {
        hS_Mmax2pdf = (TH1D*)fSDMmax2pdf->Get("Mmax2");
        hBkgMmax2pdf = (TH1D*)fBkgStagDMmax2pdf->Get("Mmax2");
      }
      
      if (type_tag==2)
      {
        hS_Mmax2pdf = (TH1D*)fSDstarGammaMmax2pdf->Get("Mmax2");
        hBkgMmax2pdf = (TH1D*)fBkgStagDstarGammaMmax2pdf->Get("Mmax2");
      }

      if (type_tag==3)
      {
        hS_Mmax2pdf = (TH1D*)fSDstarPi0Mmax2pdf->Get("Mmax2");
        hBkgMmax2pdf = (TH1D*)fBkgStagDstarPi0Mmax2pdf->Get("Mmax2");
      }

      
      Ns = hS_Mmax2pdf->GetEntries();
      Nb = hBkgMmax2pdf->GetEntries();
      ...
      dS = new RooDataHist("dS", "dS", *x, Import(*hS_Mmax2pdf));  // B->Pi+alpha
      dB = new RooDataHist("dB", "dB", *x, Import(*hBkgMmax2pdf));
    }
 ...
 if(type==3)
    {
      if (type_tag==1)
      {
        hS_Xpspdf = (TH1D*)fSDXpspdf->Get("Xps");
        hBkgXpspdf = (TH1D*)fBkgStagDXpspdf->Get("Xps");
      }
      
      if (type_tag==2)
      {
        hS_Xpspdf = (TH1D*)fSDstarGammaXpspdf->Get("Xps");
        hBkgXpspdf = (TH1D*)fBkgStagDstarGammaXpspdf->Get("Xps");
      }

      if (type_tag==3)
      {
        hS_Xpspdf = (TH1D*)fSDstarPi0Xpspdf->Get("Xps");
        hBkgXpspdf = (TH1D*)fBkgStagDstarPi0Xpspdf->Get("Xps");
      }
       
      
      
      Ns = hS_Xpspdf->GetEntries();
      Nb = hBkgXpspdf->GetEntries();
      ...
      dS = new RooDataHist("dS", "dS", *x, Import(*hS_Xpspdf)); // B->Pi+alpha
      dB = new RooDataHist("dB", "dB", *x, Import(*hBkgXpspdf)); 
    }
  ...
  if(type==1){
      ...
      hS_Mmin2pdf->Draw("HIST");
      hBkgMmin2pdf->Draw("HISTsame");
      c1->Draw();
  }
    
  if(type==2){
      TCanvas *c1 = new TCanvas("c1","c1",800,600);
      hS_Mmax2pdf->Draw("HIST");
      hBkgMmax2pdf->Draw("HISTsame");
      c1->Draw();
  }
  ...
  if(type==1)
    {
      eff_signal = (hS_Mmin2pdf->GetEntries())/Ns_generated;
      //eff_bkg= (hPitaunuMmin2pdf->GetEntries())/NB_generated;
      //eff_sm = (hSMPinuMmin2pdf->GetEntries())/Ntau_generated;
      //eff_tau = (hSMTauMmin2pdf->GetEntries())/Ntau_generated;
    }
  if(type==2)
    {
      eff_signal = (hS_Mmax2pdf->GetEntries())/Ns_generated;
  ...
  if(type==3)
    {
      eff_signal = (hS_Xpspdf->GetEntries())/Ns_generated;
  ...
  cout<<" Signal efficiency for D : "<<eff_signal<<endl;
  ...
  if (type_tag==1)
  {
    factor =  2*lum*CS_ee_BB*Br_tagD*eff_signal;
  }
  if (type_tag==2)
  {
    factor =  2*lum*CS_ee_BB*Br_tagDstarGamma*eff_signal;
  }
  if (type_tag==3)
  {
    factor =  2*lum*CS_ee_BB*Br_tagDstarPi0*eff_signal;
  }

  cout<<" factor: (2*lum*CS_ee_BB*Br_tag*eff_signal_tag) = "<<factor<<endl;
  ...
  if (type == 1){
    ...
    if (lum == 50000){
      ...
      if (type_tag==2)
      {
         w.factory("B[1078.0,900.0,1200.0]");
      }
      if (type_tag==2)
      {
         w.factory("B[3511.0,2900.0,3700.0]");
      }
  ...
  if (type == 3){
    ...
    if (lum == 50000){
      ...
      //w.var("mu")->setVal(2.5e-7);
      //w.var("mu")->setMin(0);
      w.var("mu")->setMin(-4.0e-6);
  
  
          
      // to m_x = 0.0    
      w.var("mu")->setMax(3.5e-6);
      // to m_x = 3.0
